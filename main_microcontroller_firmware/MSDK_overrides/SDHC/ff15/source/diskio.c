...
)
{
	...
	case DEV_SD:
		...
		if (MXC_SDHC_Lib_Read(buff, sector, count, MXC_SDHC_LIB_QUAD_DATA) != E_NO_ERROR)
		{
			status = RES_ERROR;
		}
		else
...
)
{
	...
	case DEV_SD:
		...
		if (MXC_SDHC_Lib_Write(sector, (void *)buff, count, MXC_SDHC_LIB_QUAD_DATA) != E_NO_ERROR)
		{
			status = RES_ERROR;
		}
		else
